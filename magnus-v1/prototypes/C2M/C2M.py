...
patternSize = (7, 7)
imgPaths = glob.glob("./Images/*")
cbPatternPath = "./Images/0.jpg"
imgSize = (400, 400)
...
def findHomography(img):
    ...
    Brief : Finds the homography matrix between an image of the current board and an image of the ideal board positions
        ...
        Returns :
            retImg (bool) : Whether did the function detect the board borders or not
    ...
    while True:
        ...
        if retImg:
            ...
            ############
            print("Please adjust your camera and remove any chess piece or obstacle")
...
def takePic():
    """
    Brief : Takes a 640x480 picture in bgr format (Used in final project)
        Returns :
    ...
    if not cap:
        print("RPi camera module not found")
    else:
...
def demo():
    ...
    prevImg = cv2.imread(imgPaths[1])  # Empty chessboard image

    showImg(prevImg, "Empty chessboard image")

    homography = findHomography(prevImg)

    showImg(applyHomography(prevImg, homography), "Empty chessboard image wraped")

    print("Place the peices on the board, Do not move the board")

    prevImg = cv2.imread(imgPaths[2])  # Full chessboard image
    ...
    curImg = cv2.imread(imgPaths[3])  # Full chessboard image after the move
    ...
    confidenceRate = [9408.88, 9860.13]
...
if __name__ == "__main__":
    moves, confidenceRate = demo()
    print("The played move/s is: ")
    print(moves)
    print(confidenceRate)
