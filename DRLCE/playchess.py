    ...
    while True:
        ...
        if mode == "h" and board.turn == color:
            ...
            if verbose:
                ...
                print(
                    "total rollouts {} Q {:0.3f} duplicate paths {} elapsed {:0.2f} nps {:0.2f}".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        usage="Play chess against the computer or watch self play games."
    )
    parser.add_argument("--model", help="Path to model (.pt) file.")
    parser.add_argument(
        "--mode", help="Operation mode: 's' self play, 'p' profile, 'h' human"
    )
    parser.add_argument("--color", help="Your color w or b")
    parser.add_argument(
        "--rollouts", type=int, help="The number of rollouts on computers turn"
    )
    parser.add_argument(
        "--threads", type=int, help="Number of threads used per rollout"
    )
    parser.add_argument(
        "--verbose", help="Print search statistics", action="store_true"
    )
