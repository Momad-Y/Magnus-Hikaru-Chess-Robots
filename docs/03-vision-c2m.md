...
**Magnus** — L2 norm, with the tile size hardcoded:
...
**Hikaru** — mean absolute difference, with the tile size derived:
...
system rejects it and asks the player to fix the position — which is why the
...
| Square → coordinates | — | `find_squares_coordinates()` |
| Camera → arm transform | — | `cam_2_arm_transformation()` |
