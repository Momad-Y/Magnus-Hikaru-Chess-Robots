...
import os  # Importing the os module to build filesystem paths
...
from engine_path import find_stockfish  # Cross-platform Stockfish lookup

# Anchor every asset path to this file's directory rather than the process
# working directory, so the app runs from anywhere on any OS.
cwd = os.path.dirname(os.path.abspath(__file__))

stockfish_path = find_stockfish()  # Locating the stockfish engine for this platform

cell_coordinates_path = os.path.join(
    cwd, "Calibration Files", "Calibration.xml"
)  # Setting the path to the cell coordinates file

DRLCE_weights_path = os.path.join(
    cwd, "DRLCE", "weights", "AlphaZeroNet_20x256.pt"
)  # Setting the path to the DRLCE weights file

motherboard_path = os.path.join(
    cwd, "Images", "Motherboard.jpg"
)  # Setting the path to the motherboard image

gui_logo_path = os.path.join(
    cwd, "Images", "icon.png"
)  # Setting the path to the GUI logo image (PNG: OpenCV cannot decode .ico)

fen_csv_path = os.path.join(
    cwd, "FEN Data", "chess_games_fen.csv"
)  # Setting the path to the fen csv
...
class chess_game:
    def __init__(self, master: tk.Tk):
        ...
        self.master.iconphoto(True, tk.PhotoImage(file=gui_logo_path))  # Setting the icon of the GUI window if the timer is for the engine
    ...
    def init_widgets(self):
        ...
        self.abdelnasser_label = tk.Label(
            self.master,
            text="Mohamed Abdelnasser's\nGithub",
            font=("Courier New", 20, "bold", "underline"),
            bg=bg_color,
            fg=link_txt_color,
            cursor="hand2",
        )  # Creating a label to display the text "Mohamed Abdelnasser's Github"

        self.abdelnasser_label.bind(
        ...
        self.hazem_label = tk.Label(
            ...
            fg=link_txt_color,
            cursor="hand2",
        )  # Creating a label to display the text "Hazem Abdelghafar's Github"
        ...
        self.raheem_label = tk.Label(
            self.master,
            text="Mohamed Elfeel's\nGithub",
            font=("Courier New", 20, "bold", "underline"),
            bg=bg_color,
            fg=link_txt_color,
            cursor="hand2",
        )  # Creating a label to display the text "Mohamed Elfeel's Github"
    ...
    def init_game(self):
        ...
        self.abdelnasser_label.place(
            relx=0.5, rely=0.75, anchor=tk.CENTER
        )  # Placing Abdelnasser's label in the GUI window
        ...
        self.empty_img = c2m.read_img(
            os.path.join(cwd, "testing", "results", "Live Empty.jpg")
        )  # Test override: replaces the camera grab above with a stored frame.
        # Was "Live Previous.jpg", which is the board WITH pieces -- wrong input for
        # the empty-board homography below. The sibling overrides pair prev_img with
        # "Live Previous" and cur_img with "Live Current", so empty_img pairs with
        # "Live Empty". Unverifiable without the rig; corrected to match the pattern.
    ...
    def display_game(self):
        ...
        self.prev_img = c2m.read_img(cwd + "/testing/results/Live Previous.jpg")  # Test
    ...
    def switch_turn(self):
        ...
        self.cur_img = c2m.read_img(cwd + "/testing/results/Live Current.jpg")  # Test
